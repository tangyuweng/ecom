...
- **Domain Layer（領域層）**: 包含核心業務實體和資料庫介面，不依賴任何框架
- **Application Layer（應用層）**: 協調業務邏輯，處理用例流程
- **Infrastructure Layer（基礎設施層）**: 實作技術細節（資料庫、JWT、遷移等）
...
2. **Repository Interface**: 在 `internal/domain/repository/` 定義介面
3. **Infrastructure**: 在 `internal/infrastructure/database/` 實作 Repository
4. **DTOs**: 在 `internal/application/dtos/` 定義資料傳輸物件
5. **Use Cases**: 在 `internal/application/usecases/` 實作業務邏輯
6. **Handlers**: 在 `internal/presentation/handlers/` 實作 HTTP 處理器
7. **Router**: 在 `internal/presentation/router/` 註冊路由
8. **Swagger**: 添加 Swagger 註解並重新生成文檔
