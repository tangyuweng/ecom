package mysql

import (
	"context"
	"errors"
	"testing"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
	"github.com/tangyuweng/ecom/internal/domain/entity"
	"github.com/tangyuweng/ecom/internal/domain/repository"
	"github.com/tangyuweng/ecom/internal/infrastructure/mysql/models"
	"gorm.io/driver/mysql"
	"gorm.io/gorm"
)

// setupTestDB 設置測試資料庫
// 注意：這需要一個真實的測試資料庫連接
func setupTestDB(t *testing.T) *gorm.DB {
	// 這裡使用測試資料庫連接
	dsn := "root:password@tcp(localhost:3308)/ecom_test?charset=utf8mb4&parseTime=True&loc=Local"
	db, err := gorm.Open(mysql.Open(dsn), &gorm.Config{})
	if err != nil {
		t.Skip("Skipping integration test: database not available")
		return nil
	}

	// 自動遷移測試表
	err = db.AutoMigrate(
		&models.UserModel{},
		&models.CategoryModel{},
		&models.ProductModel{},
		&models.CartModel{},
		&models.CartItemModel{},
		&models.OrderModel{},
		&models.OrderItemModel{},
	)
	require.NoError(t, err)

	return db
}

// cleanupTestDB 清理測試資料
func cleanupTestDB(t *testing.T, db *gorm.DB) {
	db.Exec("DELETE FROM order_items")
	db.Exec("DELETE FROM orders")
	db.Exec("DELETE FROM carts")
	db.Exec("DELETE FROM cart_items")
	db.Exec("DELETE FROM products")
	db.Exec("DELETE FROM categories")
	db.Exec("DELETE FROM users")
}

// TestWithTransaction_Success 測試事務成功提交
func TestWithTransaction_Success(t *testing.T) {
	db := setupTestDB(t)
	if db == nil {
		return
	}
	defer cleanupTestDB(t, db)

	tm := NewGormTransactionManager(db)
	userRepo := NewMysqlUserRepository(db)
	ctx := context.Background()

	// 準備測試數據
	user := &entity.User{
		Name:     "testuser",
		Email:    "test@example.com",
		Password: "hashedpassword123",
		Phone:    "1234567890",
		Role:     entity.RoleUser,
	}

	// Act - 在事務中創建用戶
	var userID string
	err := tm.WithTransaction(ctx, func(txCtx context.Context) error {
		if err := userRepo.Create(txCtx, user); err != nil {
			return err
		}
		userID = user.ID
		return nil
	})

	// Assert
	assert.NoError(t, err)

	// 驗證數據已提交到資料庫
	foundUser, err := userRepo.FindByID(ctx, userID)
	assert.NoError(t, err)
	assert.Equal(t, "testuser", foundUser.Name)
}

// TestWithTransaction_Rollback 測試事務回滾
func TestWithTransaction_Rollback(t *testing.T) {
	db := setupTestDB(t)
	if db == nil {
		return
	}
	defer cleanupTestDB(t, db)

	tm := NewGormTransactionManager(db)
	userRepo := NewMysqlUserRepository(db)
	ctx := context.Background()

	user := &entity.User{
		Name:     "testuser2",
		Email:    "test2@example.com",
		Password: "hashedpassword123",
		Phone:    "1234567890",
		Role:     entity.RoleUser,
	}

	expectedErr := errors.New("intentional error")
	var userID string

	// Act - 在事務中創建用戶後返回錯誤
	err := tm.WithTransaction(ctx, func(txCtx context.Context) error {
		if err := userRepo.Create(txCtx, user); err != nil {
			return err
		}
		userID = user.ID
		// 返回錯誤以觸發回滾
		return expectedErr
	})

	// Assert
	assert.Error(t, err)
	assert.Equal(t, expectedErr, err)

	// 驗證數據已回滾，資料庫中不應該有這個用戶
	_, err = userRepo.FindByID(ctx, userID)
	assert.Error(t, err)
	assert.Equal(t, entity.ErrUserNotFound, err)
}

// TestWithTransaction_PanicRecovery 測試 panic 處理
func TestWithTransaction_PanicRecovery(t *testing.T) {
	db := setupTestDB(t)
	if db == nil {
		return
	}
	defer cleanupTestDB(t, db)

	tm := NewGormTransactionManager(db)
	userRepo := NewMysqlUserRepository(db)
	ctx := context.Background()

	user := &entity.User{
		Name:     "testuser3",
		Email:    "test3@example.com",
		Password: "hashedpassword123",
		Phone:    "1234567890",
		Role:     entity.RoleUser,
	}
	var userID string

	// Act & Assert - 測試 panic 會被捕獲並重新拋出
	assert.Panics(t, func() {
		_ = tm.WithTransaction(ctx, func(txCtx context.Context) error {
			if err := userRepo.Create(txCtx, user); err != nil {
				return err
			}
			userID = user.ID
			panic("intentional panic")
		})
	})

	// 驗證數據已回滾
	_, err := userRepo.FindByID(ctx, userID)
	assert.Error(t, err)
	assert.Equal(t, entity.ErrUserNotFound, err)
}

// TestWithTransaction_NestedOperations 測試事務中的多個操作
func TestWithTransaction_NestedOperations(t *testing.T) {
	db := setupTestDB(t)
	if db == nil {
		return
	}
	defer cleanupTestDB(t, db)

	tm := NewGormTransactionManager(db)
	categoryRepo := NewMysqlCategoryRepository(db)
	productRepo := NewMysqlProductRepository(db)
	ctx := context.Background()

	// 準備測試數據
	category := &entity.Category{
		Name: "Test Category",
		Slug: "test-category",
	}

	product := &entity.Product{
		Name:          "Test Product",
		Description:   "Description",
		Price:         100,
		StockQuantity: 10,
		IsActive:      true,
	}

	// Act - 在同一事務中創建分類和產品
	err := tm.WithTransaction(ctx, func(txCtx context.Context) error {
		if err := categoryRepo.Create(txCtx, category); err != nil {
			return err
		}

		product.CategoryID = category.ID

		if err := productRepo.Create(txCtx, product); err != nil {
			return err
		}
		return nil
	})

	// Assert
	assert.NoError(t, err)

	// 驗證兩個實體都已創建
	foundCategory, err := categoryRepo.FindByID(ctx, category.ID)
	assert.NoError(t, err)
	assert.Equal(t, "Test Category", foundCategory.Name)

	foundProduct, err := productRepo.FindByID(ctx, product.ID)
	assert.NoError(t, err)
	assert.Equal(t, "Test Product", foundProduct.Name)
}

// TestWithTransaction_PartialRollback 測試部分操作失敗時的完整回滾
func TestWithTransaction_PartialRollback(t *testing.T) {
	db := setupTestDB(t)
	if db == nil {
		return
	}
	defer cleanupTestDB(t, db)

	tm := NewGormTransactionManager(db)
	categoryRepo := NewMysqlCategoryRepository(db)
	productRepo := NewMysqlProductRepository(db)
	ctx := context.Background()

	category := &entity.Category{
		Name: "Test Category 2",
		Slug: "test-category-2",
	}

	product := &entity.Product{
		Name:          "Test Product 2",
		Description:   "Description",
		Price:         100,
		StockQuantity: 10,
		IsActive:      true,
	}

	// Act - 創建分類成功，但產品失敗
	err := tm.WithTransaction(ctx, func(txCtx context.Context) error {
		// 第一個操作成功
		if err := categoryRepo.Create(txCtx, category); err != nil {
			return err
		}

		product.CategoryID = category.ID

		// 第二個操作成功
		if err := productRepo.Create(txCtx, product); err != nil {
			return err
		}

		// 但最後返回錯誤
		return errors.New("rollback all")
	})

	// Assert
	assert.Error(t, err)

	// 驗證所有操作都被回滾，包括第一個成功的操作
	_, err = categoryRepo.FindByID(ctx, category.ID)
	assert.Error(t, err)
	assert.Equal(t, entity.ErrCategoryNotFound, err)

	_, err = productRepo.FindByID(ctx, product.ID)
	assert.Error(t, err)
	assert.Equal(t, entity.ErrProductNotFound, err)
}

// TestGetDB 測試 GetDB 輔助函數
func TestGetDB(t *testing.T) {
	db := setupTestDB(t)
	if db == nil {
		return
	}

	t.Run("returns transaction DB when in transaction context", func(t *testing.T) {
		tm := NewGormTransactionManager(db)
		ctx := context.Background()

		err := tm.WithTransaction(ctx, func(txCtx context.Context) error {
			txDB := GetDB(txCtx, db)

			// 驗證返回的是事務 DB
			assert.NotNil(t, txDB)

			// 可以通過檢查 context value 來驗證
			ctxValue := txCtx.Value(repository.TransactionContextKey)
			assert.NotNil(t, ctxValue)
			assert.Equal(t, ctxValue, txDB)

			return nil
		})

		assert.NoError(t, err)
	})

	t.Run("returns default DB when not in transaction context", func(t *testing.T) {
		ctx := context.Background()
		resultDB := GetDB(ctx, db)

		// 驗證返回的是原始 DB with context
		assert.NotNil(t, resultDB)

		// 驗證 context 中沒有事務
		ctxValue := ctx.Value(repository.TransactionContextKey)
		assert.Nil(t, ctxValue)
	})
}
